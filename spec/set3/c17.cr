...
describe "3.17" do
  ...
  it "attacks cbc padding oracle" do
    inputs = [
      "MDAwMDAwTm93IHRoYXQgdGhlIHBhcnR5IGlzIGp1bXBpbmc=",
      "MDAwMDAxV2l0aCB0aGUgYmFzcyBraWNrZWQgaW4gYW5kIHRoZSBWZWdhJ3MgYXJlIHB1bXBpbic=",
      "MDAwMDAyUXVpY2sgdG8gdGhlIHBvaW50LCB0byB0aGUgcG9pbnQsIG5vIGZha2luZw==",
    ...
    oracle = Cryptopals::Oracles::Server.new(mode: Cryptopals::AES::Mode::CBC)
    target = Base64.decode(inputs.sample)
    result = oracle.encrypt(target)
    ciphertext = result[:encrypted].not_nil!
    iv = result[:iv].not_nil!

    oracle_fun = ->(x : Bytes, y : Bytes) { oracle.valid?(x,y) }
    broken = Cryptopals::Attacks::AES.break_cbc_padding_oracle(oracle_fun, iv, ciphertext)

    broken.should eq(target)
  end
